...
import { 
  ...
  ChefHat, Store, Target, ChevronRight 
} from 'lucide-react';

export default function ProviderSettings({ setActiveTab }) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-[#404943]">
              Manage your kitchen language preferences, profile settings, and session
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="glass-panel p-6 rounded-3xl shadow-xl space-y-4 border border-black/5 flex flex-col justify-between">
          <div className="space-y-3">
            <div className="flex items-center gap-2.5 pb-2 border-b border-black/5">
              ...
              <h2 className="font-heading font-bold text-base text-[#181a2e]">
                Kitchen & Chef Profile
              </h2>
            </div>
            <div className="space-y-1">
              <p className="text-sm font-bold text-[#181a2e]">
                {profile?.kitchen_name || 'My Kitchen'}
              </p>
              <p className="text-xs text-[#707973]">
                Chef: {profile?.provider_name || user?.email} • {profile?.mobile}
              </p>
            </div>
          </div>
          ...
          <button
            ...
            className="w-full btn-pill btn-primary text-xs py-2.5 flex items-center justify-center gap-2 cursor-pointer font-bold"
          >
        ...
        {/* 3. Kitchen Location & Service Area */}
        <div className="glass-panel p-6 rounded-3xl shadow-xl space-y-4 border border-black/5 flex flex-col justify-between">
          <div className="space-y-3">
            <div className="flex items-center gap-2.5 pb-2 border-b border-black/5">
              <MapPin className="w-5 h-5 text-[#2d6a4f]" />
              <h2 className="font-heading font-bold text-base text-[#181a2e]">
                Kitchen Location & Geofence
              </h2>
            </div>
            <p className="text-xs font-semibold text-[#181a2e] line-clamp-2">
              {providerLocation?.address || profile?.kitchen_address || 'No kitchen address set'}
            </p>
          </div>

          <div className="grid grid-cols-2 gap-2 pt-2">
            <button
              onClick={() => setActiveTab('provider-location')}
              className="btn-pill bg-[#2d6a4f]/10 hover:bg-[#2d6a4f]/20 text-[#0f5238] text-[11px] py-2 flex items-center justify-center gap-1 cursor-pointer font-bold"
            >
              <span>Map & Pin</span>
            </button>
            <button
        ...
        <div className="glass-panel p-6 rounded-3xl shadow-xl space-y-4 border border-rose-100 bg-gradient-to-b from-white to-rose-50/30 flex flex-col justify-between">
          <div className="space-y-3">
            ...
            <p className="text-xs text-[#707973]">
              Sign out from your Aapna Tiffin kitchen provider session on this device.
            </p>
          </div>
